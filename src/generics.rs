...
struct PointAnoter<T, U> {
  x: T,
  y: U,
}
impl<T, U> PointAnoter<T, U> {
  fn mixup<V, W>(self, other: PointAnoter<V, W>) -> PointAnoter<T, W> {
    PointAnoter {
      x: self.x,
      y: other.y,
    }
...
pub fn run() {
  ...
  let p3 = PointAnoter { x: 1.0, y: 2 };
  let p4 = PointAnoter { x: "Rust", y: 'a' };
  let p5 = p3.mixup(p4);
  println!("{} {}", p5.x, p5.y);
}
